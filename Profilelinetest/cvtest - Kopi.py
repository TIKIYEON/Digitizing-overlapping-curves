...
def extractIntersectionpoints(src_image):
    ...
    np.savetxt("curvepoints.txt", curve_points, fmt='%f')
    curve_a_start = np.array([0, 0])
    curve_b_start = np.array([15, 793])

    curve_a = [curve_a_start]
    curve_b = [curve_b_start]

    # Function to calculate distance
    def distance(point1, point2):
        return np.linalg.norm(point1 - point2)

    # Classify points based on proximity
    for point in curve_points[2:]:  # Skip start points
        dist_a = distance(point, curve_a[-1])  # Distance to last point in Curve A
        dist_b = distance(point, curve_b[-1])  # Distance to last point in Curve B
        if dist_a < dist_b:
            curve_a.append(np.array(point))
        else:
            curve_b.append(np.array(point))
...
def process_chunks(chunk):
    ...
    plt.axis("image")
    plt.show()
    #iio.imwrite("overlaytest4.tif",overlay)
