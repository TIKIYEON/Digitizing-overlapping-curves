...
def extractCurves(path):
    ...
    skeleton = morphology.skeletonize(dilated // 255)  # Normalize binary to 0 and 1
    ...
    def trace_curve(skeleton, start_x, start_y, intersection_points):
        ...
        while True:
            ...
            if (current_x, current_y) in intersection_points:
                # Split into a new curve at intersection
                neighbors = [neighbor for neighbor in neighbors if neighbor not in curve]
                for neighbor in neighbors:
                    curve.append(neighbor)
                break
            
            # Continue tracing the curve
            current_x, current_y = neighbors[0]  # Prioritize the first neighbor
    ...
    print(startpoints)
    #endpoints = sorted_points[mid_index:] 

    for (x, y) in endpoints:
        if (x,y) not in visited_endpoints:
            curve = trace_curve(skeleton, x, y, intersection_points)
            curves.append(curve)
            visited_endpoints.update(curve)
    print(len(curves))
