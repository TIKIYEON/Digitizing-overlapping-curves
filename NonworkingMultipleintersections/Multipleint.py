...
def extractCurves(ima):
    ...
    _, binary = cv2.threshold(image, 120, 255, cv2.THRESH_BINARY)  # Make curves white
    plt.figure(figsize=(10,6))
    plt.imshow(binary, cmap='gray')
    plt.show()
    #Skeletonize the binary image
    dilated = cv2.dilate(binary, kernel=np.ones((3,3), np.uint8), iterations=2)
    """ plt.figure(figsize=(10,6))
    plt.imshow(dilated, cmap='gray')
    plt.show() """
    eroded = cv2.erode(dilated, kernel=np.ones((3,3), np.uint8), iterations=1)
    """ plt.figure(figsize=(10,6))
